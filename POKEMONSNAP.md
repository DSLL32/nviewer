# Pokémon Snap (N64, US): ROM format specification for the level viewer

This document specifies everything needed to add *Pokémon Snap* to the viewer:
- ROM identification and versions, boot, the overlay tables, scenes and the VPK0 codec;
- the level list with in-game names, and how a course is loaded;
- the level format: world blocks, display lists, textures, render state, skybox, fog;
- the rail path and camera;
- objects: Pokémon spawns, their models at rest, props;
- collision;
- the music driver, song list and loops;
- the mapping onto `src/rom/`;
- verification evidence, open questions, and unused or hidden content.

**Evidence labels:**
- **Verified:** checked against the ROM bytes (decoded data, disassembly, script output) or the running game (emulator RAM, breakpoints, screenshots, audio captures). The method is named.
- **Decomp:** read from the public WIP decompilation (github ethteck/pokemonsnap, clone at `decomp/`), not independently re-checked where so labelled.
- **Hypothesis:** inference.

**Research material** is under `/home/n64/.ai-tmp/r49/snap/`:

| path | contents |
|---|---|
| `decomp/` | read-only clone of the decompilation (commit 1978bb5) |
| `fs/` | identification, overlay tables, loader call sites, ROM map, VPK0 (`fs/proto/vpk0.ts`, `fs/proto/snapfs.ts`) |
| `lv/` | world geometry, environment, rail path, collision; prototype level decoder and renders |
| `obj/` | spawns, Pokémon and prop models; prototype and renders |
| `mus/` | music; prototype renderer, WAVs, captures |
| `rt/` | runtime captures (RAM, frame display lists, screenshots) |
| `un/` | unused-content pass |
| `notes/*.md` | detailed notes per topic |

Unless marked, addresses are US (`NPFE`). RAM addresses are KSEG0 (0x80...); ROM offsets are into the big-endian `.z64`.

## 0. At a glance

| topic | summary |
|---|---|
| ROM | 16 MB, game code `NPFE`, CIC-6103, version byte 0. Eight other dumps (J, E, F, G, I, S, A, Snap Station) share the structure at shifted addresses (§1.1) |
| code | main uncompressed at 0x80000400 (libultra 2.0I, F3DEX2/L3DEX2 2.08H, HAL sys library). 31 `Overlay` structs (one in main, a table of 30 in app_render at ROM 0x57580) plus `dmaReadRom` constants; a 25-entry scene jump table selects the load list (§2) |
| filesystem | **none**: fixed ROM ranges per overlay/segment, position-dependent (absolute vram pointers), resolved per scene load list (§2.4) |
| codec | VPK0 (Huffman-coded LZ), used by three identified menu/intro/check blobs; every decoded level asset is raw (§3) |
| levels | 7 courses: Beach, Tunnel, Volcano, River, Cave, Valley, Rainbow Cloud (§4.3). The Pokémon Lab is 2D only; the album, report, gallery and camera check re-render course worlds for photos; the opening's 3D landscape lives in the main-menu VPK0 buffer |
| level format | WorldSetup → model/scenery blocks placed at worldPos × 100, plain F3DEX2 lists in HAL's exporter order with runtime-built segment 0x0E material lists (§5). Fog and clear colour from the WorldSetup; a camera-attached sky dome; no lighting on the world |
| rail | per-block AnimCmd scripts drive B-spline control points (arc-length parameterised); eye ≈ cart + 100, fovy 55; block-visibility schedules; 60 updates per second. The decode matches RAM on every course (§6.3) |
| objects | per-block ObjectSpawn lists (231) → per-course PokemonDef tables → PokemonInitData (DObj tree, materials, animations); lit, skinned (G_MODIFYVTX) models with near/far LOD; frame 0 of the first animation; 117 absolute spline paths; HD photo models (§7) |
| collision | height map = BSP of 2D lines → plane patches with RGB surface types; a ceiling map in Tunnel and Cave; hitbox colliders on static objects; no separate terrain-wall structure identified (§8) |
| music | classic libultra ALCSPlayer, 37 statically selected compressed-MIDI songs, `B1` music bank, 32006 Hz. **`libultra.ts` + `cseq.ts` render unchanged** with a CC 21 extra-volume adapter; the renders match captured game audio (§9) |
| difficulty | detection/resolver small; world blocks medium (display-list normalisation, material lists); rail/collision small-medium; Pokémon models medium-high (`displaylist.ts` gaps: tile masks and command order, G_MODIFYVTX, F3DEX2 light spacing); music small (§10.8) |
| unused and hidden (§13) | unreachable Japanese gallery-slideshow overlay; Snap Station's 64 preset photos and 640×480 printer compositor; a dead compressed anti-piracy helper plus the live save-sabotage path; live crash screen and dead performance/watchdog tools; two unused Beach effect sets; two unique untriggered SFX samples; no orphaned course geometry or unique course texture image |

## 1. ROM identification

All dumps are 16 MB big-endian ROMs with CIC-NUS-6103. Byte-swapped `.v64` and little-endian `.n64` dumps are normalised first (`normalizeByteOrder`).

**Verified** (`fs/romid.py` → `fs/romid.txt`):
- Shared fields: PI word 0x80371240, clock 0x0000000F, entry 0x80100400, release 0x00001449, name `POKEMON SNAP` + 8 spaces, version byte 0x3F = 0.
- IPL3 0x40-0xFFF is identical in every dump: CRC32 0x0B050EE0 (CIC-6103). CRC1/CRC2 recompute with the 6103 algorithm (seed 0xA3886759 over 0x1000-0x100FFF) and match every header.
- The header entry 0x80100400 is the usual 6103 value; the IPL3 subtracts 0x100000, and the code is linked at 0x80000400.

| dump | game code | CRC1 / CRC2 | md5 |
|---|---|---|---|
| Pokemon Snap (U) | NPFE | CA12B547 / 71FA4EE4 | fc3c9329b7cdd67cf7650abf63b9a580 |
| Pokemon Snap Station (U) | NPHE | 39119872 / 07722E9F | a9c272687dabd59c5144774b53bcc35a |
| Pokemon Snap (A) | NPFU | 7BB18D40 / 83138559 | e5a0ca3dc54b38ea7fcd927e3cffad3b |
| Pokemon Snap (E) | NPFP | 4FF5976F / ACF559D8 | f2a8106403d2bf9350bfeab08689d54a |
| Pokemon Snap (F) | NPFF | BA6C293A / 9FAFA338 | e9028f9ccc307806695dd81742d05d5d |
| Pokemon Snap (G) | NPFD | 5753720D / 2A8A884D | 144b8906dc40534cfbef6d7b994a982b |
| Pokemon Snap (I) | NPFI | C0C85046 / 61051B05 | d0453459095f69be36d675d8f743069b |
| Pokemon Snap (S) | NPFS | 817D286A / EF417416 | a45d7115be5a06fd1567f9f913c3bdf8 |
| Pocket Monsters Snap (J) | NPFJ | EC0F690D / 32A7438C | fbdd74ed68e6a0cd734562d56ccb752d |

The US sha1 `edc7c49cc568c045fe48be0d18011c30f393cbaf` is the decomp's target (**verified**).

**Detection.** The version byte is 0 in every dump, so it cannot tell dumps apart. Accept `NPFE` at 0x3B. Confirm by finding the overlay table by structure (§2.2): it is at ROM 0x57580 in the US and Station dumps.

### 1.1 Other dumps (detection level)

**Verified** (`fs/ovltables.py`, `fs/segmap.py`, `fs/reloc.py`):
- Every dump has the same structure: one Overlay struct in main, a table of 30 in app_render, three VPK0 streams, and the same scene-manager code. `fs/proto/snapfs.ts` parses all nine with one algorithm.
- No two dumps share a layout: ROM offsets and link addresses shift everywhere.

  | dump | overlay table ROM | beach_assets ROM / vram | app_level vram |
  |---|---|---|---|
  | U, Station | 0x57580 | 0x0A74E0 / 0x8011B050 (Station 0x0A7500 / 0x8011B070) | 0x80350200 |
  | A | 0x57440 | 0x0A73A0 / 0x8011BB10 | 0x80350200 |
  | E, F, G, I, S | 0x572A0 | e.g. E 0x0A6E30 / 0x8011B630 | 0x8034F1C0 (G 0x8034F1A0) |
  | J | 0x57090 | 0x0A6BA0 / 0x80119FA0 | 0x8033B220 |

- **Level content is almost entirely identical and only relinked.** Every ordinary differing word in the level asset segments is a pointer shifted by that dump's address delta (Station +0x20, E +0x5E0, J −0x10B0). The exceptions found by pointer-normalised comparison are 53 words in J `cave_assets`, all in the Cave opening rail-camera animation, and two ten-word runs omitted from an unreferenced palette-like region of E `cave_assets` (**verified** by ROM-word alignment and AnimCmd decode; §13.6).
- **Audio.** The banks and tables are identical in E/F/G/I/S/A/Station. J differs.
- **Snap Station (NPHE)** is the US game with a kiosk UI:
  - main is identical except the 7-word audio ROM-offset table (+0x8B0);
  - gallery, main menu, camera check and Oak's lab overlays are resized;
  - it adds print/kiosk strings ("Please make sure a print credit…") and drops the save prompts.

A loader for the other dumps needs only the per-dump overlay table and loader addresses, which `snapfs.ts` finds by structure. **This spec targets US**; Station could share the loader at +0x20 vram.

## 2. Boot and code

### 2.1 Boot, main, microcode
...
- `dmaReadVPK0(rom, ram)` 0x8000350C streams a VPK0 file into RAM (§3).
...
### 2.4 Position dependence and address resolution
...
1. Take the scene's load list (§2.3): the play list for the level, or the view list for `*_extra`/HD models.
...
| unk_segment_AA18E0_vpk0 | AAA610-AAA65B | 1 | 0x50 | 0x80200000 | dead anti-piracy RSP-memory check; its loader has no caller (§13) |
...
| **AE0510-AEFC10** | – | 4 gallery + 60 album preset `PhotoData` records; loaded only by Snap Station (§13) | Station pair |
...
struct WorldSetup {
  ...
  /*04*/ StaticModelEntry* staticModelTable; // id -> attach handler + payload (§7.4); NULL in 5 courses
  /*08*/ s32 unk_08;                         // 0 everywhere
  /*0C*/ CollisionModel* collisionModels;    // id -1 ends (§8.3)
...
struct WorldBlockDescriptor {           // 0x24
  ...
  /*18*/ StaticObject* staticModels;    // props attached by id through WorldSetup.staticModelTable (§7.4)
  /*1C*/ ObjectSpawn* spawn;            // Pokémon and effect spawns (§7)
  /*20*/ StaticObject* staticObjects;   // collision-bearing objects (§8.3)
};
struct WorldBlockGFX {
  ...
  /*10*/ DObjTreeNode* road;            // rail control points (§6.3)
...
**DObj trees** (`UnkEC64Arg3`, 0x2C: `{s32 id; Gfx* dl; Vec3f pos, rot, scale}`, terminated by id 18; **decomp**, used by roads, props and Pokémon, §7.3):
...
**Microcode: F3DEX2** (§2.1). **Verified** by walking every course list, including sky and props, and following DE calls. The opcode histogram:
```
...
per material:  [E3 TEXTLUT] [FC combine] [FA prim] E8
               ...
               DE 0E0000xx  (material sub-list, §5.4)  or  FD SETTIMG image
...
### 5.4 Materials and the segment 0x0E lists
...
### 5.5 Render state set by game code
...
### 5.6 Units, axes, projection

- Right-handed, Y up. Vertex units are game units; block units × 100 are game units, used by collision and spawns (**decomp**; **verified** by the ground check in §6.4 and by renders: the Beach start shows the terraced cliff left of the rails and a palm on the right, as in the game's opening straight).
...
## 6. Environment, rail path and camera

### 6.1 Fog and clear colour

Fog is `gSPFogPosition(fogMin, fogMax)` with fog colour = WorldSetup fog RGB (§5.5). With near 10 and far 25600, the viewer's `Fog` is `{color, multiplier: trunc(128000/(max−min)), offset: trunc((500−min)·256/(max−min)), near: 10, far: 25600}`:
...
- The game draws skies **opaque**. The viewer's `Sky` forces alpha blending, which is wrong for Rainbow Cloud's layered sky (§10.4).
...
Cart height minus the height-map ground along every rail sample (**verified**, `lv/dumps/ground_check.txt`): median 0.0 on Beach, 0.3 on Tunnel, 0.7 on Volcano, 0.0 on River and 2.8 on Valley, but **986 on Cave**, where the cart really falls through a shaft (§6.3). Five courses agree independently on the block placement, the animation and path decode, and the height-map conventions.
...
- **Static models** (palm, tunnel computers; §7.4) and the collision-only static objects (§8.3).
...
Which spawn function each species uses is in `id_models.tsv`. Beach Meowth and Valley Magikarp intentionally use both, depending on behavior; the exact records and initializer branches are in §13.9.
...
- **Hidden parts:** AnimCmd 15 sets DObj flags: bit 1 skips the node's lists, bit 2 its subtree. Pikachu's lightning nodes 1/2/14, balloon node 15 and apple node 20 are hidden at spawn this way (728 → 343 triangles; **verified** in RAM and by animation/list decode). There is no surfboard node; Beach object 1006 supplies the surfing splash/wake assembly (§13.9).
- **Materials:** the same `Texture` records and segment 0x0E sub-lists as the world (§5.4), with material animations (e.g. blinking eyes by image index) at frame 0.
...
- **In ROM:** 117 spawns have an `InterpData` path (§6.3 format): Beach 25, Tunnel 14, Volcano 10, River 26, Cave 20, Valley 22. They are B-splines or linear, with 2-29 points.
...
### 8.1 Height map and ceiling map
...
The height maps live in the course code segments. The HeightMap struct pointers (§5.1) resolve through the code segment's vram; lead re-checked on Beach: 0x802CE1B0 → ROM 0x566220, 0x802ED498 → ROM 0x585508.
...
### 8.4 What the viewer should draw
...
- HAL modified the player (`csplayer.c`, `seqplayer.c` and others, marked "Only in Pokemon Snap" in the decomp); the one change that matters for rendering is in §9.4.
...
- **Reverb.** `AL_FX_CUSTOM` with `auCustomFXParams1` (ROM 0x42B50, **verified** bytes): 8 sections on a 10400-sample (325 ms) delay line. A 14-section preset exists, but nothing selects it (§13). Songs send CC 91 between 2 and 64. Cave, Tunnel and Rainbow Cloud add an extra player FX mix (20, 20, 25; **decomp**).
...
## 10. Mapping onto the viewer

### 10.1 Detection and game object

- **`src/rom/index.ts`:** add `case 'NPFE'`, calling `openPokemonSnap(rom)`. The other dumps (`NPHE`, `NPFU`, `NPFP`, `NPFF`, `NPFD`, `NPFI`, `NPFS`, `NPFJ`) should throw "only Pokémon Snap (U) is supported", or be accepted later through `snapfs.ts`'s per-dump structure search (§1.1).
- **`src/rom/types.ts`:** extend `Game.id` with `'pokemonsnap'`. The title is `Pokémon Snap`.
- **Level list:** the seven courses of §4.3. Use `LevelKind` `campaign` for Beach…Valley and `bonus` for Rainbow Cloud, or one group "Courses".
- **Confirmation:** after matching the game code, confirm with the overlay-table structure at ROM 0x57580 (30 strict Overlay structs, §2.2) and throw on a mismatch.

### 10.2 New modules (suggested)

| file | contents | port from |
|---|---|---|
| `src/rom/pokemonsnap/fs.ts` | overlay table, scene load lists, per-scene vram → ROM resolver (§2.4); VPK0 only if menu art is wanted | `fs/proto/snapfs.ts`, `fs/proto/vpk0.ts` (tested on all 9 dumps) |
...
| `src/rom/pokemonsnap/gfx.ts` | material records, synthesis of the segment 0x0E lists, list normalisation (§10.3) | `lv/proto/gfx.ts` |
...
| `src/rom/pokemonsnap/objects.ts` | spawns, Pokémon and prop models at rest (§7) | `obj/proto/` |
| `src/rom/pokemonsnap/music.ts` | song table, adapter onto `music/libultra.ts` + `music/cseq.ts` (§9.4) | `mus/proto/snapmusic.ts` |
...
The stored lists do not decode correctly with `displaylist.ts` as it is (**verified**, §5.3-§5.4). The five gaps:
1. **Segment 0x0E material lists** do not exist in ROM. The loader synthesises them from the Texture records (§5.4) at animation frame 0.
...
10. **Lighting:** lit presets with the level Lights1 (ambient 100, light 180, direction (30,30,30) rotated with the camera, §7.3), via the `lighting` option. Bake it with a fixed direction, e.g. from the start camera.
...
- **Instances:** blocks at translate(worldPos × 100); props at `(worldPos + pos)·100` with the RPY matrix (§7.4); spawns per §7.
- **Layers:**
  - "course" (main): model blocks;
  - "scenery" (background): scenery blocks;
  - "objects": props and Pokémon (§7);
  - "height map", "ceiling map", "hitboxes": collision, hidden;
  - "spawns", "rail path": markers, hidden.
- **Environment:** `fog` per §6.1; `clearColor` = bg RGB; `skies` = the sky mesh (camera-relative); `camera` = the start eye/target of §6.3 with fovY 55 (the game's play fovy, **verified**).
...
Build the height map and ceiling map cells as meshes in 'collision' layers (§8.4), with per-vertex colours from the surface RGB and alpha ~170, blended, no depth write, no culling. Hitboxes can be simple primitive meshes.
...
Track list: the 37 rows of §9.3, `index` = song id, `name` = viewer name (with the id as a two-digit prefix, like GoldenEye). Loops follow the per-track loop markers. One-shot songs get no loop. **No changes to `src/rom/music/` are required.** An `extraVolume` option in `renderSequence` would replace the bank copy.

### 10.7 Out of scope for a first version

The opening's 3D model/material package (inventoried in §13.1) could be ported as a viewer pseudo-level; the credits and photo scenes also remain out of scope. The Pokémon Lab is 2D only (§4.3).
...
| blocks, materials, list normalisation | medium | prototype ~600 lines; the display-list gaps of §10.3 are the main work |
...
| objects: Pokémon models at frame 0 | medium-high | DObj trees with four payload types, skinning with G_MODIFYVTX, near LOD, AnimCmd frame 0 with hidden-node flags, lit presets; `displaylist.ts` changes in §10.3 items 6-11 |
| music | small | reuse; adapter ~90 lines |

## 11. Verification evidence

Paths are relative to `/home/n64/.ai-tmp/r49/snap/`. Running-game checks used the headless mupen64plus with the debugger core (`/home/n64/nviewer/EMULATOR.md`), one instance at a time, each in its own run directory (`mus/run-mus`, `rt/run-rt`).

### 11.1 ROM, code, overlays, codec
...
### 11.2 Level format, environment, rail, collision
...
| Renders = game | `mus/cap/cap1.raw` + `cap1.log` (32006 Hz), `mus/compare.py` (table in §9.4); flute table ROM 0x5232D0 (lead re-checked) |
...
| Load lists = §2.3 | loader breakpoint log `rt/loads.txt` (116 calls: boot, 7 courses, Lab, menu); `createWorld` a0 = the §5.1 WorldSetups |
...
### 11.6 Emulator and process hygiene

- The music capture used `mus/run-mus`; its recorded PID is no longer live. The course/runtime capture used `rt/run-rt`; its recorded PID is no longer live. Both sessions were explicitly stopped and checked by their owning agents (**verified** from their session logs and a final PID/process sweep).
- The resumed unused-content work was static and started no emulator. The investigation therefore stayed at zero live Snap emulator sessions, within the one-session limit across the full agent subtree (**verified** by repeated scoped process sweeps).
- **Process defect found during the resumed pass:** discovery agents initially used filesystem-wide or overly broad `find`/`grep`/`rg` searches instead of `$NVIEWER_ROMS`, named repositories and `command -v`. The coordinator terminated six broad search processes; the lead terminated a lingering recursive `grep` (PIDs 2250324/2250330) and a later high-CPU all-`r49` `rg` (PIDs 2283092/2283101; no Snap child claimed it). All children were then constrained to named Snap paths, known ROM paths and bounded tools, and final task-local checks were clean (**verified** from process command lines and child cleanup reports). Future investigations should state allowed roots and process ownership in the dispatch itself, then require a PID-tagged cleanup check.

### 11.7 Unused and hidden content
...
### 12.2 Level format and environment
...
- Decorative names for several generic props in 1008-1034 remain deliberately unresolved; the six Pokémon Signs, controllers, spawners and major effects are identified in §13.9.
...
- SFX IDs 126 and 305 are unique, untriggered samples; their semantic identities have not been assigned by ear (§13.7).

## 13. Unused and hidden content
...
### 13.1 Negative result: no retained cut course
...
### 13.2 Dead scene 24 and the Snap Station printer path
...
### 13.3 Anti-piracy code and save sabotage
...
### 13.4 Preset photographs retained in the standard ROM
...
### 13.5 Course-asset and effect remnants
...
- J `cave_assets` differs meaningfully in only 53 aligned words: pointer motion around an
  ...
  AnimCmd decode; exact rows in `notes/unused.md` §7.1).
...
- The preset photo block is shared by every non-J retail dump but loaded only by Station
  (§13.4), a concrete example of kiosk content retained in ordinary regional ROMs.
...
### 13.8 Debug and untranslated material
