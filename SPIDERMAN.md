# Spider-Man (N64, USA): ROM formats for the level viewer

> Research specification. This investigation was read-only with
> respect to `/home/n64/nviewer`. Unless a paragraph says otherwise, multi-byte
> values are big-endian, ROM offsets address the normalized `.z64` image, and
> RAM addresses are N64 virtual addresses.

Evidence labels used throughout:

- **Verified—ROM:** directly checked in the supplied ROM bytes or decoded data.
- **Verified—disassembly:** established from the supplied ROM's MIPS code.
- **Verified—RAM/frame/audio:** measured in a bounded emulator session.
- **Cross-port lead:** information from another platform or an external tool,
  not accepted as N64 evidence until independently checked.
- **Hypothesis:** a bounded inference that still needs verification.

Research material is under `/home/n64/.ai-tmp/r49/spiderman/`:

| path | contents |
|---|---|
| `fs/` | bounded ERZ/master-directory extractor, full ROM maps, boot disassembly |
| `levels/` | complete stage census, geometry/TRG survey, static renders |
| `environment/` | render/environment disassembly, backdrop catalog, runtime-capture analyzer |
| `music/` | bank/schedule analyzers and effect/arrangement render prototypes |
| `hidden/` | reference audit and recording-studio/full-screen-image contact sheets |
| `emulator/session1/` | partial audio evidence plus explicitly invalidated controller-race evidence |
| `emulator/session2/` | controlled L1A1 frame, RDRAM and display-list captures |

## 0. At a glance

| topic | summary |
|---|---|
| ROM | 32 MiB US `NSLE`, revision 0, CIC-6102; one supplied retail build |
| storage | resident loader + 995,056-byte ERZ2-compressed main image; an 8-group master directory contains 1,789 leaves and 3,285 nonempty logical assets; 30 late overlays |
...
| music | Nintendo Sound Tools sample/effect engine at 22,047 Hz; adaptive level schedules select prerecorded ADPCM fragments rather than conventional MIDI-like sequence files |
| difficulty | medium-high: compression and base geometry are bounded; faithful placement overlays, camera/background behavior and adaptive music scheduling are the main work |
| unused/hidden | incomplete L3A1a trigger, declaratively unreferenced recording-studio model family, one dangling L8A2 background reference, complete training content and live debug/cheat menus (§11) |

**Verified—ROM/disassembly** except the items explicitly left as hypotheses in
their detailed sections. Reproducible evidence is indexed in §9.

## 1. ROM identification and versions

The supplied file is a 32 MiB, big-endian US ROM. **Verified—ROM.**

| field | value |
|---|---|
| path | `/data/software/ai-scratch/Spider-Man (U) [!].z64` |
| size | 33,554,432 bytes (`0x2000000`) |
| PI word | `80371240` |
| clock / entry / release | `0000000F` / `80000400` / `00001444` |
| CRC1 / CRC2 | `A60ED171` / `3D85D06E` |
| internal name | `SPIDERMAN` |
| game code / revision | `NSLE` / 0 |
| MD5 | `7f1991b8861e7e532ec21ecf2af82191` |
| SHA-1 | `382c9c5d4e416444c4f6f9f4f6a5914d679fedf1` |
| SHA-256 | `feff90ed1201c91ff167d66958048e61c192c9d6a756ddb98f799017ac9cd25c` |
| IPL3 | CIC-NUS-6102 (`crc32(ROM[0x40:0x1000]) = 90BB6CB5`) |

The host file's 1996 modification time is not treated as game-build evidence.

### 1.1 Other releases and detection

Only the supplied US revision has been examined. **Verified—ROM.** A loader
should normalize byte order, require game code `NSLE`, require revision 0, and
then validate the master-directory shape in §2.3. The whole-ROM hash is useful
for exact-build diagnostics but should not be the only detector. Any future
prototype or regional image must be treated as a separate address/table profile
until its structure is compared; no compatibility claim is made here.

## 2. Boot, executable layout, filesystem and compression
...
### 2.2 Table grammar and master directory
...
### 2.4 ROM map and byte accounting
...
### 3.1 Extra, alternate and incomplete stages
...
## 4. Geometry, textures and collision

### 4.1 Model bundles and placement
...
### 4.2 Geometry groups and materials
...
### 5.1 Lighting and model render state
...
## 6. Objects, placements and animation
...
### 7.3 Decode, scheduling and loops
...
- all geometry-bearing extra stages in §3.1, while incomplete TRGs fail softly
...
### 11.1 Incomplete alternate L3A1 section
...
### 11.2 Declaratively unreferenced recording studio
...
### 11.4 Candidate unused music stems
...
### 11.5 Live cheats, debug and easter eggs
