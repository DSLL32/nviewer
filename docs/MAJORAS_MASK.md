# The Legend of Zelda: Majora's Mask — Nintendo 64 ROM format specification

This manual describes the shipped data formats needed to identify, extract, and
present The Legend of Zelda: Majora's Mask content. Claims state their evidence inline; unsupported
...
| Compression | Yaz0 per file. |
| Graphics microcode | F3DEX2 with animated-material records. |
| Geometry | Scene headers, room headers, and mesh types 0–2. |
| Textures | RDP textures, TLUTs, and scene-defined animated materials. |
...
| Levels | 102 populated USA scenes; 113 debug scene IDs. |
| Memory requirement | Expansion Pak. |
| Viewer support | USA retail and PAL debug source formats. |

### 1.2 ROM identification

| Release | NAME | Game code | Revision | Size | CRC1 | CRC2 | SHA-1 | CIC | Build |
|---|---|---|---:|---:|---|---|---|---|---|
| USA | `ZELDA MAJORA'S MASK` | `NZSE` | 0 | 32 MiB (`0x2000000`) | `5354631C` | `03A2DEF0` | `d6133ace5afaa0882cf214cf88daba39e266c078` | CIC-6105 | 00-07-31 17:04:16 |
| PAL debug | `ZELDA MAJORA'S MASK` | `NZSP` | 1 | 64 MiB (`0x4000000`) | `9FC385E5` | `3ECC05C7` | `55541662a192c66e34a011d4bf6f4a0ec69899ae` | CIC-6105 | 00-09-29 09:29:05 |
...
AnimatedMaterial list (scene command 0x1A): eight-byte entries. The segment used is `abs(segment) + 7`; the list ends after the first negative segment, and an initial zero means no list.

| Offset | Size | Type | Field | Description |
|---:|---:|---|---|---|
| 0x00 | 1 | s8 | segment | Signed segment selector and termination flag |
| 0x01 | 1 | u8 | padding | Alignment |
| 0x02 | 2 | s16 | type | Parameter type below |
| 0x04 | 4 | u32 | params | Segmented pointer to type-specific parameters |


| type | params | static frame (step 0) |
|---|---|---|
| 0 tex scroll | Four-byte texture-scroll parameters below | TexScroll(0, 0, w, h) |
| 1 two-layer scroll | 2 × the above | TwoTexScroll with offsets 0 |
| 2 colour, no interpolation | Color-animation parameters below | prim = primColors[0] (alpha × ratio), env = envColors[0] |
...
| 5 texture cycle | Texture-cycle parameters below | segment = list[indices[0]] |

Texture-scroll parameters, four bytes:

| Offset | Size | Type | Field | Description |
|---:|---:|---|---|---|
| 0x00 | 1 | s8 | xStep | Horizontal increment. |
| 0x01 | 1 | s8 | yStep | Vertical increment. |
| 0x02 | 1 | u8 | width | Texture width. |
| 0x03 | 1 | u8 | height | Texture height. |

Color-animation parameters, 16 bytes:

| Offset | Size | Type | Field | Description |
|---:|---:|---|---|---|
| 0x00 | 2 | u16 | keyFrameLength | Cycle length. |
| 0x02 | 2 | u16 | keyFrameCount | Key-frame count. |
| 0x04 | 4 | u32 | primColors | Pointer to five-byte R,G,B,A,lodFrac records. |
| 0x08 | 4 | u32 | envColors | Pointer to four-byte RGBA records, or zero. |
| 0x0C | 4 | u32 | keyFrames | Pointer to u16 key-frame positions. |

Primitive-color record, five bytes:

| Offset | Size | Type | Field | Description |
|---:|---:|---|---|---|
| 0x00 | 4 | u8[4] | color | R,G,B,A. |
| 0x04 | 1 | u8 | lodFrac | LOD fraction. |

Environment-color record, four bytes:

| Offset | Size | Type | Field | Description |
|---:|---:|---|---|---|
| 0x00 | 4 | u8[4] | color | R,G,B,A. |

Texture-cycle parameters, 12 bytes:

| Offset | Size | Type | Field | Description |
|---:|---:|---|---|---|
| 0x00 | 2 | u16 | keyFrameLength | Cycle length. |
| 0x02 | 2 | u8[2] | padding | Alignment. |
| 0x04 | 4 | u32 | list | Texture-pointer array. |
| 0x08 | 4 | u32 | indices | Byte-index array. |

Census MM US (**verified**, `scene-mm/report-mm-us.txt`): type 1 × 165, 2 × 3, 4 × 1, 5 × 1, and 42 empty lists; no type 0
or 3. Renderer: `scenes/drawcfg.ts mmAnimatedMaterials`, **verified** on South Clock Town and Termina Field (*Scenes, display lists, environment*).
