# Mario Kart 64 — Nintendo 64 ROM format specification

This manual describes the shipped data formats needed to identify, extract, and
present Mario Kart 64 content. Claims state their evidence inline; unsupported
...
| Property | Value |
|---|---|
| Asset organization | Fixed course table; six resources per course and no general filesystem. |
| Compression | MIO0 course data, vertices, display-list bytecode, and individual textures. |
| Graphics microcode | F3DEX and F3DLX 0.95. |
| Geometry | Packed vertices expanded into segment 4 and bytecode expanded into segment 7 display lists. |
| Textures | Individually MIO0-compressed textures assembled into segment 5. |
| Collision | Tagged display-list groups expanded into triangle records. |
| Music driver | Nintendo EAD sequence engine. |
| Audio microcode | `aspMain` “NEAD MK”. |
| Sample encoding | Nintendo ADPCM. |
| Levels | 20 courses plus the award ceremony; optional source-archive Town course. |
| Memory requirement | Base 4 MiB. |
| Viewer support | USA revision 0; other revisions documented. |

### 1.2 ROM identification

| Release | NAME | Game code | Revision | Size | CRC1 | CRC2 | SHA-1 | CIC | Build |
|---|---|---|---:|---:|---|---|---|---|---|
| USA | `MARIOKART64` | `NKTE` | 0 | 12 MiB (`0xC00000`) | `3E5055B6` | `2E92DA52` | `579c48e211ae952530ffc8738709f078d5dd215e` | CIC-6102 | — |
| Europe V1.0 | `MARIOKART64` | `NKTP` | 0 | 12 MiB (`0xC00000`) | `C3B6DE9D` | `65D2DE76` | `a729039453210b84f17019dda3f248d5888f7690` | CIC-6102 | — |
| Europe V1.1 | `MARIOKART64` | `NKTP` | 1 | 12 MiB (`0xC00000`) | `2577C7D4` | `D18FAAAE` | `f6b5f519dd57ea59e9f013cc64816e9d273b2329` | CIC-6102 | — |
| Japan V1.0 | `MARIOKART64` | `NKTJ` | 0 | 12 MiB (`0xC00000`) | `6BFF4758` | `E5FF5D5E` | `afeeec65b9a03f0cb8ec92f9ba7a9f0122e8bd0e` | CIC-6102 | — |
| Japan V1.1 | `MARIOKART64` | `NKTJ` | 1 | 12 MiB (`0xC00000`) | `C9C3A987` | `5810344C` | `9f439457585146a4e1da7e1dd9104f7f94381688` | CIC-6102 | — |
...
| 0x8015F580 / 0x8015F588 | `gCollisionMesh` / `gCollisionMeshCount` (*Collision*) |
...
- course data through `gCourseTable` (racing segment rodata, *gCourseTable*);
- course textures by offsets stored in each course's texture list, relative to the `other_textures` ROM block (*Texture lists and segment 5*);
...
| 0x966260-0xBE9160 | audio data (*Music*) | audio heap |
...
- **Verified, exhaustive decode:** `fs/proto/extract.ts` validates all 20 course entries, MIO0 boundaries, CourseVtx
  ...
  leaked generated course-header counts. Japanese mismatches are version differences recorded in *Version differences*, not decode errors.
...
- the segment-6 layout of most courses (smaller, e.g. Mario Raceway 38048 vs 38904 bytes) and the section tables in segment 9, so the *Course geometry and render state*/*Collision* addresses do not apply; J V1.1 also changes segment 6 of Frappe Snowland and Rainbow Road at equal size;
...
`setup_race` calls `load_course(id)` when the course changes, then `course_generate_collision_mesh()` (*Collision*):
...
5. `decompress_vtx`: the CourseVtx stream at segF+0 is decoded into the pool, then expanded (*Vertices: CourseVtx to Vtx*) to `gHeapEndPtr -= align16(count x 16)` = segment 4.
...
- **Credits**: race courses drawn with `render_course_credits` (one whole-course list per course, incomplete, *What makes up a static course*).
...
| 1 Choco Mountain | [cam] SHADE 7004608; fog (*Environment: sky, fog, clouds, camera*), 2-cycle, MODULATERGB+PASS2 / FOG_SHADE_A+OPA2: 7005A70, 7000828, 70008E0, 7005868; section table; no cull, FOG+TEX_EDGE2, DECAL+PASS2: 7000448, 70005D8; cull 7000718 | - |
...
| 10 Toad's Turnpike | fog (*Environment: sky, fog, clouds, camera*): I+PASS2 / FOG_SHADE_A+OPA2 section table; FOG+TEX_EDGE2, DECAL+PASS2: 7000000, 7000068, 70000D8 | - |
...
| G_SETFOGCOLOR | `3C3C3CFF` (9); other fog colours are set by game code (*Environment: sky, fog, clouds, camera*) |
| G_TEXTURE | on `BB000001 FFFFFFFF`, off `BB000000 00010001` |

- **Cut-outs and translucency:** TEX_EDGE (coverage x alpha) is an alpha cut-out (fences, trees, signs); XLU and XLU_INTER blend (water, ice, Rainbow Road) with the vertex alpha of *Load-time edits and animation*. `displaylist.ts`'s existing blend/cutout classification gets these right.
...
- **Inherited state:** in Moo Moo Farm (543 triangles) and D.K.'s Jungle Parkway (572), packed lists take their texture or render mode from the calling section list, and different section variants supply different values; the prototype keeps the first draw (*Open questions and hypotheses*). A viewer must run section lists with their texture loads, not the packed lists alone.
...
##### TOWN archive payloads

Unlike the retail course resources described in *Filesystem and compression*, TOWN survives as generated C arrays and symbol/address metadata,
...
##### July packed display-list variant
...
##### Viewer package and presentation
...
- **Verified, ROM bytes/disassembly:** sky colour, cloud/star, path-size, path-pointer, object-position, lighting,
  ...
  is taken only from the matching decompilation, *Objects and the kart path* through *Environment: sky, fog, clouds, camera* retain the **Decomp** label.
...
Differences from `sf64.ts` (everything else matches STARFOX.md *Surface types*):
...
- Results (16) plays section A 64 times (52.6 minutes) before a section B (*Unused and hidden content*); the viewer loops section A.
