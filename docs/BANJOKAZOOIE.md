...
The only other unreferenced asset in the 164-model environment block is USA V1.0 `0x149C` (ROM `0x746348`, stored `0x158` bytes, decoded `0x4BC`). Its display list ends immediately, so it draws no triangles; its 20 vertices and 12 unique collision faces define an outer floor and an inner walled, ceilinged volume. It has no map, sky, ActorInfo or resolved setup-prop reference. A literal `0x149C` search of the boot block and all 32 decoded code/data images found no occurrence, though computed runtime IDs cannot be excluded. The decompilation leaves the ID unnamed and comments only on its invisible walls and floors; that comment does not establish its use. The asset survives in later revisions under shifted IDs. Its purpose remains unknown.
