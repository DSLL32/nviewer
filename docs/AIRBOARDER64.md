# Air Boarder 64 — Nintendo 64 ROM format specification

This manual describes the shipped data formats needed to identify, extract, and
present Air Boarder 64 content. Claims state their evidence inline; unsupported
...
|---|---|---|---:|---:|---|---|---|---|---|
| Japan | `ｴｱｰﾎﾞｰﾀﾞｰ64` | `NABJ` | 0 | 8 MiB (`0x800000`) | `6C45B60C` | `DCE50E30` | `4a70c9ca027ecc8d05337993204e1ef76f5f0ac9` | CIC-6102 | — |
| Europe/PAL | `AIR BOARDER 64` | `NABP` | 0 | 8 MiB (`0x800000`) | `27C425D0` | `8C2D99C1` | `2171fe2c0a9253cba56828f24a5e6153726c1516` | CIC-6102 | — |
...
The ten `0x24`-byte descriptors start at J `0x27C34`; resident function `0x80028D18` loads them. PAL uses the same logical images but different bounds (`shared/e_rom_map.tsv`): resident ends `0x2B0A0`, overlay load bases are `0x8009DC50` and `0x80123340`, final overlay ends `0xA5090`. [evidence: disassembly]

Common gameplay at J `0x800A1F70` loads stage overlay `course+4`, so legal course IDs 0–5 map directly to overlays 4–9. Overlay 2's preload loops and nearby rider/board/menu strings establish its front-end role; overlay 3 selects the course, board/rider assets, common course data and renderer. [evidence: disassembly]

The ROM contains `RSP Gfx ucode F3DLX 1.21 Yoshitaka Yasumoto Nintendo.` at J `0x2A3A0` and `F3DLX.Rej 1.21` at `0x2ABA0` (P `0x2A350` / `0x2AB50`). [evidence: ROM bytes]
...
| 63 | likely unused alternate recorded-input stream (*Likely unused alternate controller recording*) |
...
Every direct archive call in resident and all ten bounded overlays was inventoried in `shared/j_archive_xrefs.tsv`; computed bases 6, 22, 38 and 50 cover all indexed groups. [evidence: deterministic decoding]
...
Each stage overlay's initializer at runtime `0x80127180` contains the fixed course-archive load, so all six are proven live. Course names/order and the menu labels `LECTURE`, `STREET WORK`, `TIME ATTACK`, `COIN`, `FREE RUN` are corroborated by retail screens; session2 directly observed `STREET WORK`, `GREEN PARK`, `LEVEL 1`. [evidence: disassembly, RAM]
...
The exact 75 descriptor selections, including aliased selections, and every record are in `levels/course_catalog.json` and `levels/placement_obj/`. Coin counts independently match the retail mode's reported totals, establishing family B's semantics. [evidence: ROM bytes, deterministic decoding]
...
`levels/green_park_structural.png`, generated by a thin adapter into nviewer's existing `tools/render/raster.ts`, shows the same rectangular park, central openings, ramps and surrounding terrain as the retail selection/demo imagery. A backface-culling variant remains coherent with counter-clockwise fronts. This is a structural winding/scale check, not a claim of exact camera/material parity. [evidence: deterministic decoding]
...
High-Y faces near 10,000 in Green/Snow are valid indexed faces, not parser overruns; boundary/sentinel geometry is their likely purpose. Exported `levels/collision_obj/*.obj` preserves source quads. Emit all collision in a hidden-by-default `collision` layer. [evidence: deterministic decoding, hypothesis]
...
Use these as default viewer cameras. If a regional/version variation later needs a fallback, use a decoded start/marker location, 65° vertical FOV, near 10 and the course far clip. Do not invent a static camera field in the course header. [evidence: RAM, disassembly]
...
Authored tick bounds, before regional frame quantization, are: ID0 `0+3072`; ID1 `0+6339`; ID2 `192+7680`; ID3 `192+9216`; ID4 `7488+384`; ID5 `192+9216`; ID6 `192+6720`; ID7 `0+7680`. ID4's 104-second introduction and short loop are independently encoded by all ten channels, not an analyzer artifact. [evidence: ROM bytes, deterministic decoding]

Gameplay selector J `0x800E0310` uses overlay-3 table at ROM `0x899E0` / RAM `0x800EA170`, mapping `[0,1,2,3,5,6,7]`. Course-overlay calls prove all course associations; state `0x8005008A = state+0x202` distinguishes 1P/2P Green Park. Session2 corroborated Green Park 1P (`a0=3`) and Lost Forest (`a0=5`). Front-end selector J `0x800AF604`, while overlay 2 is loaded, indexes table ROM `0x404A4` / RAM `0x800B4224` mapping `[0,4,3]`; direct calls use slots 0 and 1. [evidence: disassembly, RAM]
...
Adjacent used J64 is loaded by overlay 3 `0x800B01B4`; input-source case 2 at `0x800B32D4` consumes one indexed `u16` per frame into the same internal input word as live controls. The used recording is region-specific (2,500 J vs 2,400 P samples), consistent with VI-timed attract/demo input. [evidence: disassembly]
...
`music/wav/` contains eight 32,000-Hz stereo audition renders driven by the existing proven old-libmus engine. They cover song start through one loop plus about three seconds. They validate parsing and loop seams but are not emulator/DAC bit comparisons. The research harness temporarily substitutes a profile at build time; production must replace that ad hoc substitution with the declarative core above. [evidence: deterministic decoding]
...
- music: course association from *Music player*; front-end tracks remain available in the music box.
...
6. Determine whether any non-libmus direct code path selects one of the 52 wave records not referenced by the eight songs or 257 indexed effects (*Narrow unused-wave candidates*).
