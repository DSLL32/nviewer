...
static const char *l_DebugLogPath = NULL;    // file for debugger console output (default: stdout)
...
static void printUsage(const char *progname)
{
    printf("Usage: %s [parameters] [romfile]\n"
           ...
           "    --debug-log (file)     : write the debugger console's output to (file) instead of stdout\n"
...
static m64p_error ParseCommandLineMain(int argc, const char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "--debug-log") == 0 && ArgsLeft >= 1)
        {
            l_DebugLogPath = argv[i+1];
            i++;
        }
...
int main(int argc, char *argv[])
{
    ...
    if (l_LaunchDebugger)
    {
        ...
        if (l_DebugLogPath != NULL && debugger_set_output(l_DebugLogPath) != 0)
        {
            DebugMessage(M64MSG_WARNING, "couldn't open debugger log '%s', using stdout.", l_DebugLogPath);
        }
