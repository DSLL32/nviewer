...
static int   l_TakeRingScreenshot = 0;   // Same, for the next periodic screenshot of the ring
...
int main_set_core_defaults(void)
{
    ...
    ConfigSetDefaultInt(g_CoreConfig, "ScreenshotRingSize", 0, "Periodically save screenshots to a circular buffer of this many files (<rom>-ring-NNNN.png, the newest also as <rom>-latest.png) in the screenshot directory. 0 disables it");
    ConfigSetDefaultInt(g_CoreConfig, "ScreenshotRingIntervalMs", 1000, "Emulated milliseconds between periodic screenshots when ScreenshotRingSize is set");
...
static void video_plugin_render_callback(int bScreenRedrawn)
{
    ...
    // same for periodic screenshots of the ring
    if (l_TakeRingScreenshot)
    {
#ifdef M64P_OSD
        if (!bOSD || bScreenRedrawn)
#endif /* M64P_OSD */
        {
            TakeRingScreenshot(l_CurrentFrame);
            l_TakeRingScreenshot = 0;
        }
    }
...
void new_vi(void)
{
    ...
    if (ScreenshotRingTick(g_dev.vi.expected_refresh_rate))
        l_TakeRingScreenshot = 1;
